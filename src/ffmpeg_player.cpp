...
void FFmpegPlayer::_bind_methods() {
    ...
    // خصائص loop و volume بشكل صحيح لتجنب الخطأ
...
bool FFmpegPlayer::load_video(const String &path) {
    ...
    // إعداد الصورة الأولية لتجنب خطأ Texture غير مُهيّأ
    current_texture.instantiate();
    Ref<Image> img = memnew(Image());
    img->create(video_width, video_height, false, Image::FORMAT_RGB8);
...
void FFmpegPlayer::stop()  { playing = false; seek(0.0); }
...
void FFmpegPlayer::_process(double delta) {
    ...
    if (duration > 0.0 && position >= duration) {
        if (looping) seek(0.0);
        else { playing = false; emit_signal("video_finished"); return; }
    }
        ...
        if (packet->stream_index == video_stream_idx) {
            if (avcodec_send_packet(video_codec_ctx, packet) == 0) {
                while (avcodec_receive_frame(video_codec_ctx, frame) == 0) {
                    ...
                    Ref<Image> img = memnew(Image());
                    img->create(video_width, video_height, false, Image::FORMAT_RGB8, pba);
                    current_texture->create_from_image(img);
